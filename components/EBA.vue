...
export default {
  ...
  mounted () {
    ...
    Window.bscene = this.scene;
    Window.bengine = this.engine;
    ...
    BABYLON.SceneLoader.ImportMeshAsync(
      ...
      this.mainMesh = loadedMeshes[1]; // assumes __root__ is zero

      // Carregando o JSon do Baking
      const fileToFetch = '/bakedAnim/' + filename.replace(/\.[^/.]+$/, '.json');
      // console.log('Fetching: ' + fileToFetch);
      return fetch(fileToFetch);
    }).then((response) => {
      if (!response.ok) {
        // Constroi o Baking porque não existe
        const baker = new BABYLON.VertexAnimationBaker(this.scene, this.mainMesh);
        baker.bakeVertexData(animationRanges).then((vertexData) => {
          ...
          a.setAttribute('download', filename.replace(/\.[^/.]+$/, '.json'));
          a.click();
